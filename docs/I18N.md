...
| `orgs`          | Organization management: creation, switching, leaving, email domain restrictions |
| `members`       | Member management: inviting, removing, contact info, role types, directory |
...
| `legal`         | Legal pages: terms of service, privacy policy, principles |
| `notifications` | Notification system: titles, actions, role assignments, policies |
| `decisions`     | Decision journal: tracking changes and audit history             |
| `glossary`      | Glossary page: core concepts and terminology definitions         |
...
- **Never omit diacritical marks**: See the dedicated section below

### Diacritical Marks and Accents

**All translations must include proper diacritical marks (accents) for their respective language.** Omitting accents produces incorrect, unprofessional text and can change meaning.

This applies to every language that uses diacritics, including but not limited to:

| Language  | Common Diacritics | Examples of correct vs incorrect |
|-----------|-------------------|----------------------------------|
| French    | é è ê à ù ç î ô | ✓ `décider` ✗ `decider`, ✓ `hiérarchie` ✗ `hierarchie`, ✓ `coopérative` ✗ `cooperative` |
| Spanish   | á é í ó ú ñ | ✓ `organización` ✗ `organizacion`, ✓ `decisión` ✗ `decision` |
| Italian   | à è é ì ò ù | ✓ `perché` ✗ `perche`, ✓ `responsabilità` ✗ `responsabilita` |
| Ukrainian | combining diacritics, soft sign, etc. | Use proper Ukrainian orthography |

**Rules:**

1. **Always write fully accented text.** Every translated string must use the correct diacritical marks as required by the target language's orthography. Unaccented text is considered a bug.

2. **Review existing translations for missing accents.** When editing a translation file, scan nearby entries for stripped accents and fix them in the same change.

3. **Use a spell checker or native speaker review.** If you are unsure about the correct accenting of a word, look it up. Common French words that are frequently written without accents include:
   - `à` (preposition, vs `a` the verb)
   - `où` (where, vs `ou` meaning or)
   - `rôle`, `équipe`, `décision`, `hiérarchie`, `secrétaire`, `responsabilité`, `intégrée`, `défini`, `spécifique`, `coopérative`, `différence`, `présence`

4. **Never accept ASCII-only text for accented languages.** If a translation file for French, Spanish, Italian, or any other accented language contains only ASCII characters (no characters above U+007F), it is almost certainly missing its diacritical marks and must be corrected.
